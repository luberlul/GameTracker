...
import { useIgdbSearch } from "@/hooks/use-igdb";
...
export function SearchModal({ open, onClose, onImport }: SearchModalProps) {
  ...
  const search = useIgdbSearch(debouncedQuery);
  ...
  return (
    <>
      <Dialog
        open={open && !selected}
        onClose={onClose}
        title="Importar do IGDB"
        description="Busque um jogo na base de dados do IGDB e importe automaticamente."
        size="lg"
      >

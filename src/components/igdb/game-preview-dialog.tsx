...
import { useIgdbGame } from "@/hooks/use-igdb";
...
export function GamePreviewDialog({ suggestion, onBack, onConfirm }: Props) {
  const { data, isLoading, error } = useIgdbGame(suggestion?.id ?? null);
            ...
            {data && (
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  ...
                  <Stat
                    label="Franquias"
                    value={data.franchises.join(", ") || "—"}
                  />
                  <Stat label="Lançamento" value={data.released ?? "—"} />
                </div>
